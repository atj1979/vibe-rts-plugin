...
export class PerformanceMonitor {
  constructor() {
    // Frame timing - track wall-clock time BETWEEN rendered frames, not within frame
    this.frameTimes = [];
    this.maxSamples = 60; // Track last 60 frames
    this.lastFrameTime = performance.now(); // Time when last frame finished rendering
   ...
   */
  begin() {
    // Measure time since LAST frame completed (not within this frame)
    const now = performance.now();
    const timeSinceLastFrame = now - this.lastFrameTime;

    // Track wall-clock delta between rendered frames
    this.frameTimes.push(timeSinceLastFrame);
    if (this.frameTimes.length > this.maxSamples) {
      this.frameTimes.shift();
    }

    // Update FPS based on time between frames
    const avgFrameTime =
   ...
   */
    ...
    // Record when this frame finished (for next frame's delta calculation)
