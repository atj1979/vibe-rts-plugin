...
export class BuildingSystem {
  ...
  placeBuilding(type, x, z, team = 0, skipConstruction = false) {
    ...
    console.log(
      `[BuildingSystem] Placed ${type} at (${x}, ${z}) for team ${team}`,
    );
  ...
  completeProduction(building) {
    ...
    if (unit) {
      ...
      if (building.hasRallyPoint) {
        ...
        console.log(
          `[BuildingSystem] Unit moving to rally point (${building.rallyPoint.x.toFixed(1)}, ${building.rallyPoint.z.toFixed(1)})`,
        );
      }
    }
      ...
      buildings.forEach((building, index) => {
        ...
        if (building.state === BuildingState.CONSTRUCTING) {
          tempColor.lerp(
            ...
            1 - building.constructionProgress,
          );
        }
  ...
  updateSelectionIndicators() {
    ...
    this.allBuildings.forEach((building) => {
      if (
        building.isSelected &&
        building.isAlive &&
        !this.selectionIndicators.has(building)
      ) {
