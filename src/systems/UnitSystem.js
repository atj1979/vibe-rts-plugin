...
export class UnitSystem {
  ...
  createInstancedMeshes() {
    ...
    Object.values(UnitType).forEach((type) => {
      ...
      console.log(
        `[UnitSystem] Created instanced mesh for ${type} (max: ${maxUnitsPerType})`,
      );
    });
  }
  ...
  spawnUnit(type, x, z, team = 0) {
    ...
    console.log(
      `[UnitSystem] Spawned ${type} at (${x}, ${z}). Total: ${this.stats.totalUnits}`,
    );
  ...
  commandMove(units, x, z) {
    ...
    console.log(
      `[UnitSystem] Commanded ${units.length} units to move to (${x}, ${z})`,
    );
  }
